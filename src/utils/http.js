...
const handleCode = (code, msg) => {
  switch (code) {
    case invalidCode:
      ...
      if (loginInterception) { // 请确保loginInterception已经在你的应用上下文中定义
        location.reload();
      }
      break;
...
class HttpClient {
  constructor() {
    ...
    // 添加响应拦截器
    this.instance.interceptors.response.use(this.handleResponse, this.handleErrorResponse);
  }
  ...
  handleResponse(response) {
    ...
    if (code === 200) {
      ...
      return data;
    } else if (headers['content-type'] && headers['content-type'].includes('application/octet-stream')) {
  ...
  handleErrorResponse(error) {
    ...
    if (error.response) {
      ...
      this.publish('requestError', error.response.data || { message: 'An unexpected error occurred.' });
    } else if (error.request) {
      // 请求已发出但未收到响应
      console.error('No response received:', error.request);
      this.publish('requestError', { message: 'No response received from the server.' });
    } else {
      // 设置请求时发生了一些问题
